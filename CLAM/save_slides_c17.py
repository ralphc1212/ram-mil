...
if __name__ == '__main__':
    for i in range(10):
        base_path = 'c17_patch_dir'
        train_val_data = []
        path = 'c17_splits/task_1_tumor_vs_normal_75/' + 'splits_{}.csv'.format(i)
        ...
        new_feat_path = os.path.join('c17_slide_dir', str(i),'train')
        if not os.path.exists(new_feat_path):
            os.makedirs(new_feat_path)
        att_path = os.path.join('c17_attention_dir', str(i))
        ...
        for emb_file in train_val_data:
